...
def indexar_documentos(config):
    ...
    try:
        ...
        if not curso_objetivo:
            logger.error(f"No se encontró el curso '{config['moodle']['target_course']}'")
                        ...
                        if 'fileurl' in contenido:
                            ...
                            try:
                                ...
                                if texto:
                                    ...
                                    logger.warning(f"No se pudo extraer texto de {nombre_archivo}")
...
def iniciar_web(config, host='0.0.0.0', port=5000, debug=True):
    """
...
def main():
    ...
    group.add_argument('--index', action='store_true', help='Indexar documentos de Moodle')
    group.add_argument('--chat', action='store_true', help='Iniciar chat interactivo')
    group.add_argument('--web', action='store_true', help='Iniciar aplicación web')
    group.add_argument('--fine-tune', action='store_true', help='Realizar fine-tuning del modelo')
